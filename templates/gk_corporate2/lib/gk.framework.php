...
class GKTemplate {
    ...
    */
	...
	public function getToolsOverride() {
		...
		if (isset($tools_override[$ItemID])) {
		    ...
		    if (isset($tools_override[$option])) {
		        ...
		        return ($this->getParam('tools', '1') == '1') ? true : false;
		    }
		}
	}
    ...
    public function generatePadding($block) {
		...
		// gkMainBlock
    	if($block == 'gkLeftColumn') return 'gkPaddingTBLR';
    	if($block == 'gkRightColumn') return 'gkPaddingTBLR';
    	if($block == 'gkContentColumn') { return 'gkPaddingTBLR'; }
    	// Content
    	if($block == 'gkInset1') return '';
    	if($block == 'gkInset2') return 'gkPaddingL';
    	if($block == 'gkComponentWrap') return ($this->modules('inset1')) ? 'gkPaddingL' : '';
    	// left column
    	if($block == 'gkLeftTop') return 'gkPaddingTBL';
    	if($block == 'gkLeftMiddle') return 'gkPaddingTBL';
    	if($block == 'gkLeftLeft') return 'gkPaddingTBL';
    	if($block == 'gkLeftRight') return 'gkPaddingTBL';
    	if($block == 'gkLeftBottom') return 'gkPaddingTBL';
    	// right column
		if($block == 'gkRightTop') return 'gkPaddingTBLR';
		if($block == 'gkRightMiddle') return 'gkPaddingTBLR';
		if($block == 'gkRightLeft') return ($this->modules('right_right')) ? 'gkPaddingTBL' : 'gkPaddingTBLR';
		if($block == 'gkRightRight') return 'gkPaddingTBLR';
		if($block == 'gkRightBottom') return 'gkPaddingTBLR';
		// main column
		...
		if($block == 'gkContentMainbody') {
			if($this->modules('right_top + right_bottom + right_left + right_right')) {
				$this->addCSSRule('#gkContentMainbody { margin-right:10px; }');
			}
		...
		if($block == 'gkMainbodyBottom') {
			if($this->modules('inset1 and inset2')) {
				return ($this->modules('mainbody_top') || $this->checkComponent() || $this->checkMainbody()) ? 'gkPaddingBLR' : 'gkPaddingTBLR';
			} elseif($this->modules('inset1')) {
				return ($this->modules('mainbody_top') || $this->checkComponent() || $this->checkMainbody()) ? 'gkPaddingBLR' : 'gkPaddingTBLR';
			} elseif($this->modules('inset2')) {
				return ($this->modules('mainbody_top') || $this->checkComponent() || $this->checkMainbody()) ? 'gkPaddingBLR' : 'gkPaddingTBLR';
    ...
    public function socialApiParser($embed_mode = false) {
    	// fb login
    	if(($this->getParam('fb_login', '0') == 1 || $this->getParam('fb_like', '0') == 1 || $this->getParam('fb_facepile', '0') == 1) && !$this->browser->get('mobile')) {
    		$this->addJS('http://connect.facebook.net/'.$this->getParam('fb_lang', 'en_US').'/all.js');
    	}
    	// FB login
    	if(!($this->getParam('fb_api_id', '') != '' && $this->getParam('fb_login', '0') == 1) || $this->browser->get('mobile')) {
    		// clear FB login
    		GKParser::$customRules['/<fb:login-button(.*)fb:login-button>/mi'] = '';
    	}
    	...
    	if(($option == 'com_content' && $view == 'article' && in_array($id, $excluded_articles, false)) ||
    		...
    		$is_excluded = true;
            // clear SocialAPI div
    		GKParser::$customRules['/<gavern:social(.*?)gavern:social>/mi'] = '';
    	} else {
    	...
    	// FB facepile
    	if($this->getParam('fb_facepile', '0') == 1 && !$is_excluded && !$this->browser->get('mobile')) {
    		$facepile_attributes = '';
    		// configure FB facepile
    		$facepile_attributes .= ' max_rows="'.$this->getParam('fb_facepile_max_rows', '1').'"';
    		$facepile_attributes .= ' width="'.$this->getParam('fb_facepile_width', '500').'"';
    		if($this->getParam('fb_api_id', '') != '') $facepile_attributes .= ' app_id="'.$this->getParam('fb_api_id', '').'"';
    		
    		GKParser::$customRules['/GK_FB_FACEPILE_SETTINGS/'] = $facepile_attributes;
    	} else {
    		// clear FB facepile
    		GKParser::$customRules['/<fb:facepile(.*?)fb:facepile>/mi'] = '';
    	}
    	// Twitter
    	if($this->getParam('tweet_btn', '0') == 1 && !$is_excluded && !$this->browser->get('mobile') && $option == 'com_content' && $view == 'article') {
    		...
    		GKParser::$customRules['/<a href="http:\/\/twitter.com\/share"(.*?)\/a>/mi'] = '';
    	}
    	// Digg
    	if($this->getParam('digg_btn', '0') == 1 && !$is_excluded && !$this->browser->get('mobile')) {
    		...
    		// clear Twitter buttons
    		GKParser::$customRules['/<a class="DiggThisButton(.*?)\/a>/mi'] = '';
    	}
    	// Google Buzz
    	if($this->getParam('buzz_btn', '0') == 1 && !$is_excluded && !$this->browser->get('mobile') && $option == 'com_content' && $view == 'article') {
    		// add Google Buzz JS
    		$this->addJS('http://www.google.com/buzz/api/button.js');
    		// configure Google Buzz buttons    		  
    		$buzz_btn_attributes = ' data-button-style="' . $this->getParam('buzz_btn_style', 'normal-count') . '"';    		  
    		GKParser::$customRules['/GK_BUZZ_BTN_SETTINGS/'] = $buzz_btn_attributes;
    	} else {
    		// clear Google Buzz buttons
    		GKParser::$customRules['/<a href="http:\/\/www.google.com\/buzz\/post"(.*?)\/a>/mi'] = '';
    	}
    	// Delicious
    	if($this->getParam('delicious_btn', '0') != 1 || $is_excluded || $this->browser->get('mobile')) {
    		// clear Delicious buttons
    		GKParser::$customRules['/<a href="http:\/\/www.delicious.com\/save"(.*?)\/a>/mi'] = '';
    	}
		// Instapaper
		if($this->getParam('instapaper_btn', '0') != 1 || $is_excluded || $this->browser->get('mobile')) {
			// clear Delicious buttons
			GKParser::$customRules['/<a href="http:\/\/www.instapaper.com\/hello2(.*?)\/a>/mi'] = '';
		}
    }
