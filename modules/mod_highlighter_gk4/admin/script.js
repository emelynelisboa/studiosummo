window.addEvent("domready",function(){
	...
	$('MOD_HIGHLIGHT_GK4_BASIC_SETTINGS-options').addEvent('click', function(){
		if($('MOD_HIGHLIGHT_GK4_BASIC_SETTINGS-options').hasClass('pane-toggler')) {
			(function(){ $$('.pane-slider').setStyle('height', 'auto'); }).delay(750);
		}
	});
	//
	var selector = $('jform_params_data_source');
	//
	$('jform_params_com_categories').getParent().setStyle('display', (selector.value == 'com_categories') ? 'block' : 'none');
	$('jform_params_com_articles').getParent().setStyle('display', (selector.value == 'com_articles') ? 'block' : 'none');
	$('jform_params_xmlfile').getParent().setStyle('display', (selector.value == 'xmlfile') ? 'block' : 'none');
	// 
	...
	// limits
	$('jform_params_title_limit_type').inject($('jform_params_title_limit_type').getParent().getNext(), 'bottom');
	$('jform_params_title_limit').getParent().getPrevious().dispose();
	$('jform_params_desc_limit_type').inject($('jform_params_desc_limit_type').getParent().getNext(), 'bottom');
	$('jform_params_desc_limit').getParent().getPrevious().dispose();
	...
	// demo link
	new Element('a', { 'href' : 'http://mootools.net/demos/?demo=Transitions', 'target' : '_blank',  'id' : 'gkDemoLink', 'html' : 'Demo'  }).inject($('jform_params_animation_fun'), 'after');
	// help link
